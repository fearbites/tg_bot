...
function App() {
  ...
  // Получение данных авторизации из URL и отправка на сервер для аутентификации
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const telegramAuthData = Object.fromEntries(urlParams.entries());

    axios.post('http://localhost:3000/auth/telegram', telegramAuthData)
      .then(response => {
  ...
  // Сохранение прогресса пользователя при изменении очков или энергии
  useEffect(() => {
    if (userId) {
      const updateProgress = () => {
        axios.post('http://localhost:3000/updateProgress', { userId, points, energy })
          .catch(error => {
